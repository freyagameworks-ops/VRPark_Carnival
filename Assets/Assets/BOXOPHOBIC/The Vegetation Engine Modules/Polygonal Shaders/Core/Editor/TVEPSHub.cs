...
namespace TheVegetationEnginePolygonal
{
    public class TVEPSHub : EditorWindow
    {
        float GUI_HALF_EDITOR_WIDTH = 220;

        string autorunPath;
        string assetFolder = "Assets/BOXOPHOBIC/The Vegetation Engine Modules/Polygonal Shaders";
        ...
        static TVEPSHub window;

        //Vector2 scrollPosition = Vector2.zero;

        [MenuItem("Window/BOXOPHOBIC/The Vegetation Engine/Hub/Polygonal Shaders", false, 1005)]
        public static void ShowWindow()
        {
            window = GetWindow<TVEPSHub>(false, "Polygonal Shaders Module", true);
        ...
        void OnEnable()
        {
            ...
            searchFolders = AssetDatabase.FindAssets("Polygonal Shaders");

            for (int i = 0; i < searchFolders.Length; i++)
            {
                if (AssetDatabase.GUIDToAssetPath(searchFolders[i]).EndsWith("Polygonal Shaders.pdf"))
                {
                    assetFolder = AssetDatabase.GUIDToAssetPath(searchFolders[i]);
                    assetFolder = assetFolder.Replace("/Polygonal Shaders.pdf", "");
                }
            }
            ...
            autorunPath = assetFolder + "/Core/Editor/TVEPSHubAutoRun.cs";
            ...
            bannerText = "Polygonal Shaders Module";
        ...
        void OnGUI()
        {
            ...
            {
                EditorGUILayout.HelpBox("Welcome to the Polygonal Shaders Module for the Vegetation Engine! Choose the render pipeline used in your project to make the shaders compatible with the current pipeline!", MessageType.Info, true);
            ...
            {
                EditorGUILayout.HelpBox("The Polygonal Shaders Module is installed for the " + pipelineCurrent + "! Use the additional setting below to restart the render pipeline setup.", MessageType.Info, true);
        ...
        void DrawToolbar()
        {
            ...
            {
                EditorGUIUtility.PingObject(AssetDatabase.LoadAssetAtPath<Object>(assetFolder + "/Demo/Demo Polygonal.unity"));
            }
            GUILayout.Space(-1);
            ...
            {
                Application.OpenURL("https://assetstore.unity.com/packages/vfx/shaders/the-vegetation-engine-polygonal-shaders-210347#reviews");
            }
        ...
        void SetDefineSymbols()
        {
            ...
            if (!defineSymbols.Contains("THE_VEGETATION_ENGINE_POLYGONAL"))
            {
                defineSymbols += ";THE_VEGETATION_ENGINE_POLYGONAL;";
