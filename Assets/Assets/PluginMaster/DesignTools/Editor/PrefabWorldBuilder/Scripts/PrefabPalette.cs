...
namespace PluginMaster
{
    public class PrefabPalette : UnityEditor.EditorWindow, ISerializationCallbackReceiver
    {
        ...
        private void OnEnable()
        {
            ...
            AutoSave.QuickSave();
            var paletteFiles = System.IO.Directory.GetFiles(PWBData.palettesDirectory, "*.txt", System.IO.SearchOption.AllDirectories);
        ...
        private void PasteThumbnailSettings(object idx)
        {
            ...
            void Paste(MultibrushSettings brush)
            {
                ...
                ThumbnailUtils.UpdateThumbnail(brush, true);
            }
            if (PaletteManager.SelectionContains((int)idx))
            {
        ...
        private void BrushMouseEventHandler(BrushInputData data)
        {
            ...
            else if (Event.current.type == EventType.MouseDrag)
            {
                ...
                if (Event.current.control)
                {
                    ...
                    UnityEditor.DragAndDrop.objectReferences = new Object[] { PaletteManager.selectedBrush.GetItemAt(0).prefab };
            ...
            else if (data.eventType == EventType.DragPerform && !Event.current.control)
            {
                _moveBrush.to = data.index;
                bool toTheRight = data.mouseX - data.rect.center.x > 0;
                if (toTheRight) ++_moveBrush.to;
            ...
            else if (data.eventType == EventType.DragExited && !Event.current.control)
            {
                _showCursor = false;
                draggingBrush = false;
                _moveBrush.to = -1;
